...
const BookingForm = ({ userDetails, handleOpenSnackBar, onBookingSuccess}) => {
    ...
    const validateForm = () => {
        ...
        if (!appointmentDate || new Date(appointmentDate) < new Date()) {
            tempErrors.appointmentDate = 'Please select a date and time.';
            isValid = false;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            handleOpenSnackBar('Your Appointment booked successfully!');
            ...
            handleOpenSnackBar('Appointment booking failed. Please try again');
        }finally {
            setIsBooking(false);
        }
    };
